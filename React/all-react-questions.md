...
18. ### What is the main goal of React Fiber?
...
21. ### What is the difference between createElement and cloneElement?
...
25. ### Does the lazy function support named exports?
...
26. ### Why React uses `className` over `class` attribute?
...
29. ### How to apply validation on props in React?
...
32. ### What are the recommended ways for static type checking?
...
`useMemo` is a React hook that memoizes the result of a computation. It's used to optimize performance by avoiding unnecessary recalculations of values.

Key points:

- It takes a function and an array of dependencies as arguments.
- The function is only re-executed if one of the dependencies changes.
- It's useful for expensive calculations or when you want to prevent unnecessary re-renders.
...
`React.memo` is a higher-order component that memoizes functional components. It's used to prevent unnecessary re-renders of components when their props haven't changed.

Key points:

- It only checks for prop changes by default (shallow comparison).
- You can provide a custom comparison function as a second argument.
- It's useful for optimizing performance in components that render often with the same props.
...
`useCallback` is a hook that returns a memoized version of a callback function. It's used to optimize performance by preventing unnecessary re-creation of functions.
...
35. ### Why we need to be careful when spreading props on DOM elements?
...
37. ### How you implement Server Side Rendering or SSR?
...
38. ### How to enable production mode in React?

    You should use Webpack's `DefinePlugin` method to set `NODE_ENV` to `production`, by which it strip out things like propType validation and extra warnings. Apart from this, if you minify the code, for example, Uglify's dead-code elimination to strip out development only code and comments, it will drastically reduce the size of your bundle.

    **[⬆ Back to Top](#table-of-contents)**

39. ### Do Hooks replace render props and higher order components?

    Both render props and higher-order components render only a single child but in most of the cases Hooks are a simpler way to serve this by reducing nesting in your tree.

    **[⬆ Back to Top](#table-of-contents)**

40. ### What is a switching component?
...
42. ### Why should component names start with capital letter?
...
43. ### Are custom DOM attributes supported in React v16?
...
44. ### What is React proptype array with shape?
...
45. ### What is the difference between React and ReactDOM?

    The `react` package contains `React.createElement()`, `React.Component`, `React.Children`, and other helpers related to elements and component classes. You can think of these as the isomorphic or universal helpers that you need to build components. The `react-dom` package contains `ReactDOM.render()`, and in `react-dom/server` we have _server-side rendering_ support with `ReactDOMServer.renderToString()` and `ReactDOMServer.renderToStaticMarkup()`.

    **[⬆ Back to Top](#table-of-contents)**

46. ### Why ReactDOM is separated from React?
...
47. ### Why you can't update props in React?
...
The `useReducer` hook is a React hook that provides an alternative to `useState` for managing complex state logic in functional components. It's especially useful when you have state transitions that depend on the previous state or when the next state value depends on the previous one.
...
In this example, `useReducer` is used to manage a simple counter state. The `counterReducer` function defines how the state should be updated based on different actions. The component uses the `dispatch` function to trigger state updates by sending actions to the reducer.
...
Both `useState` and `useReducer` are hooks in React used for state management, but they have different use cases and characteristics:

1. **Complexity**:

   - `useState`: Simpler and more straightforward for managing simple state.
   - `useReducer`: Better for managing more complex state logic, especially when the next state depends on the previous one.

2. **State Structure**:

   - `useState`: Typically used for independent pieces of state.
   - `useReducer`: More suitable for managing state objects with multiple sub-values.

3. **Update Logic**:

   - `useState`: State updates are usually done directly.
   - `useReducer`: State updates are handled through a reducer function, allowing for more structured state transitions.

4. **Predictability**:

   - `useState`: Can become less predictable with multiple setState calls.
   - `useReducer`: Provides more predictable state transitions, as all updates are processed through the reducer.

5. **Debugging**:

   - `useState`: Debugging can be harder as state updates are scattered.
   - `useReducer`: Easier to debug as all state logic is centralized in the reducer function.

6. **Performance**:

   - `useState`: Might cause unnecessary re-renders if not used carefully.
   - `useReducer`: Can be more performant for complex state updates.

7. **Code Organization**:

   - `useState`: State logic is typically co-located with the component.
   - `useReducer`: Allows separation of state logic from the component, improving maintainability.

8. **Testing**:
   - `useState`: Testing state changes can be more challenging.
   - `useReducer`: Reducer functions are pure and easier to test in isolation.
...
In general, use `useState` for simple state management and `useReducer` for more complex state logic or when you need more control over state transitions.

**[⬆ Back to Top](#table-of-contents)**

267. ### How does context works using useContext hook?

The `useContext` hook in React provides a way to consume context in functional components. It simplifies the process of accessing context values without the need for render props or higher-order components. Here's how it works:
...
4. **Update context:**
   When the value passed to the Provider changes, all components using that context will re-render with the new value.
...
In this example, the `ThemeContext` is created and provided by the `App` component. The `ThemedButton` component uses the `useContext` hook to access the current theme and the function to update it, without the need for prop drilling.

The `useContext` hook simplifies the process of consuming context, making it easier to share data that can be considered "global" for a tree of React components.
